...
class DailyRoutineControllerTest {
    ...
    fun `daily routine get and update flow works`() {
        mockMvc.perform(get("/api/v1/routines/daily"))
            ...
            .andExpect(jsonPath("$.data.energyScore").exists())
            .andExpect(jsonPath("$.data.recoveryScore").exists())
            ...
            .andExpect(jsonPath("$.data.signals").isArray)
            .andExpect(jsonPath("$.data.focusMode.title").exists())
